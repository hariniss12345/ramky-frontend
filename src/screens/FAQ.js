...
export function FAQ() {
  ...
  const data = [
    {
      question: 'I paid a token amount. Do I still pay 10% as first installment?',
      answer: 'No, it’s 10% minus the token/booking amount already paid.',
    },
    {
      question: 'Invoice says 7.5% GST, but only 5% charged?',
      answer: '7.5% = GST on land + building. 5% GST is for building only.',
    },
  ];
      ...
      <ScrollView style={commonStyles.scrollContent}>
        ...
        <View style={styles.qaSection}>
          {data.map((item, index) => (
            <View key={index}>
              <TouchableOpacity
                style={[styles.card, activeQuestion === index && styles.activeCard]}
                onPress={() => toggleAnswer(index)}
              >
                <View style={styles.questionRow}>
                  <Text style={[styles.question, activeQuestion === index && styles.activeQuestionText]}>
                    {item.question}
                  </Text>
                  <Image
                    source={require('../../assets/icons/Shape.png')}
                    style={[styles.inlineIcon, activeQuestion === index && { tintColor: '#fff' }]}
                  />
                </View>
              </TouchableOpacity>
